Obj back_sm,clear_sm,background_sb;
...
void input_sb() {
        while (SDL_PollEvent(&e)) {
                ...
                if (e.type == SDL_MOUSEBUTTONDOWN) {
                        if ((mousex>=back_sm.dest.x)
                            &&(mousex<=back_sm.dest.x+back_sm.dest.w)
                            &&(mousey>=back_sm.dest.y)
                            &&(mousey<=back_sm.dest.y+back_sm.dest.h)) {
                                switch(lastpage) {
                                case 0:
                                        status = 0;
                                        break;
                        ...
                        if ((mousex>=clear_sm.dest.x)
                            &&(mousex<=clear_sm.dest.x+clear_sm.dest.w)
                            &&(mousey>=clear_sm.dest.y)
                            &&(mousey<=clear_sm.dest.y+clear_sm.dest.h)) {
                                if(openfile(true)) {
...
void scoreboard(){
        ...
        // clearscore
        clear_sm.setDest(240,500,100,100);
        clear_sm.setSource(0,0,1500,1500);
        clear_sm.setImage("data/clearscore.png",ren_sb);
        //background_sm
        background_sb.setDest(0,0,350,600);
        background_sb.setSource(0,0,1500,1500);
        background_sb.setImage("data/images.jpg",ren_sb);
        ...
        while (running_sb) {
                ...
                SDL_RenderCopyEx(ren_sb,background_sb.tex,&background_sb.src,&background_sb.dest,0,NULL,SDL_FLIP_NONE);
