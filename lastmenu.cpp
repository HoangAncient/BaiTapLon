Obj playagain_lm, highscore_lm, firstmenu_lm, saveinfo, gameover_lm, background_lm;
...
void input_lm()
{
	...
	while (SDL_PollEvent(&e) != 0)
	{
		...
		case SDL_MOUSEBUTTONDOWN:
			if ((mousex >= playagain_lm.dest.x)
                && (mousex <= playagain_lm.dest.x + playagain_lm.dest.w)
                && (mousey >= playagain_lm.dest.y)
                && (mousey <= playagain_lm.dest.y + playagain_lm.dest.h))
			{
				status = 2;
			...
			if ((mousex >= highscore_lm.dest.x)
                && (mousex <= highscore_lm.dest.x + highscore_lm.dest.w)
                && (mousey >= highscore_lm.dest.y)
                && (mousey <= highscore_lm.dest.y + highscore_lm.dest.h))
			{
				status = 5;
				lastpage = 1;
				SDL_DestroyRenderer(ren_lm);
			...
			if ((mousex >= saveinfo.dest.x)
                && (mousex <= saveinfo.dest.x + saveinfo.dest.w)
                && (mousey >= saveinfo.dest.y)
                && (mousey <= saveinfo.dest.y + saveinfo.dest.h))
			{
...
void lastmenu()
{
	...
	//background_lm
	background_lm.setDest(0, 0, 350, 700);
	background_lm.setSource(0, 0, 2000, 2000);
	background_lm.setImage("data/background_g.png", ren_lm);
	...
	while (running_lm)
	{
		...
		SDL_RenderCopyEx(ren_lm, background_lm.tex, &background_lm.src, &background_lm.dest, 0, NULL, SDL_FLIP_NONE);
